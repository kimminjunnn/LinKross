...
5. 관찰 가능한 상태 변화 묘사 및 명사형 종결: 모든 문장의 끝은 반드시 '~됨', '~함', '~표시됨', '확인' 등 명사형으로 끝내세요. ('~한다', '~된다' 서술형 종결 절대 금지). 또한 "버튼 텍스트가 마감으로 변경됨", "에러 문구가 노출됨" 등 눈에 보이는 UI 컴포넌트의 상태 변화를 명확히 서술하세요.
...
8. 세션 유지 및 연타/중복 클릭 방지 검증 분리: 
   - "새로고침 후에도 로그인/세션 유지"는 로그인 성공과 별도의 독립 DoD로 분리하세요.
   - "저장/요청 버튼 연타(빠른 다중 클릭) 시 동일 데이터가 1건만 생성됨 확인"과 "동일 조건 중복 시 안내창 표시"를 각각 개별 DoD로 분리하세요.
9. 시스템 오류·빈 상태(Empty State) 필수 반영: 원문에 오류 처리, 로딩, 데이터 없음 요구사항이 있다면 이를 누락하지 말고 "서버 오류 시 오류 안내 및 재시도 버튼 표시됨", "주문이 없을 때 빈 상태(Empty State) 화면 표시됨"을 반드시 독립 DoD로 반영하세요.
