-- SOW 작성부터 연습하기 위한 프로젝트 시드.
--
-- Supabase SQL Editor에서 한 번 실행한다. 전체가 하나의 트랜잭션이라 중간에
-- 실패하면 통째로 롤백된다. 여러 번 실행하면 그 횟수만큼 프로젝트가 생기므로,
-- 연습이 끝나면 만들어진 프로젝트 id를 확인해 두는 편이 좋다.
--
-- 확정된 시연 프로젝트(`seed_asset_rental_project.sql`)는 이미 SOW와 검수 이력이
-- 붙어 있어 SOW 작성 화면을 처음부터 다시 볼 수 없다. 그래서 참여자와 저장소
-- 관계만 같게 두고 프로젝트를 하나 더 만든다. 여기서 만든 것은 시연 확정본이
-- 아니므로 검수 결과나 프리셋 확정 근거로 쓰지 않는다.
--
-- 참여자 (확정본과 동일)
...
--
-- 어디까지 만드는가
--   프로젝트 등록 -> 요구사항 등록 -> 수행 제안서 제출 -> 선정 -> 저장소 연결.
--   SOW와 마일스톤은 만들지 않는다. 그 뒤부터는 화면에서 직접 한다.
--
-- 요구사항 원문은 `eval/presets/asset-rental.txt`를 그대로 넣는다. SOW 작성
-- 화면의 업무 상세에 붙여넣는 텍스트가 프리셋 원문과 같아야 프리셋이 붙고,
-- 완료조건 47개가 LLM 호출 없이 확정된 문장으로 나온다. 요구사항에 다른 글이
-- 들어 있으면 그것을 붙여넣게 되고, 그때는 평소의 LLM 분석 경로로 간다.

do $seed$
declare
  v_company    uuid := '97ef85c7-dea9-41ed-9907-17a7815633ca';
  v_freelancer uuid := '0a93c5d0-2d40-40af-90a5-8ed34179f0de';
  v_title      text := '사내 비품 대여 관리 프로젝트 (SOW 연습)';
...
-- ===========================================================================
--
-- 방금 만든 연습 프로젝트가 나온다. 여러 행이면 시드를 그만큼 실행한 것이다.
...
where p.company_id = '97ef85c7-dea9-41ed-9907-17a7815633ca'
  and p.title = '사내 비품 대여 관리 프로젝트 (SOW 연습)'
