...
`/opportunities`, `/company/assessments`, `/freelancer/applications`는 `projects`/`project_requirement_versions`/`proposals`/`selections`와 RLS를 통해 실제 데이터를 사용한다. 프로젝트 첨부파일도 비공개 Storage에 저장하고 모집 중인 공개 자료만 시간 제한 URL로 노출한다. 구형 하드코딩 데이터 모듈은 제거했다.
