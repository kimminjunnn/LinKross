...
AI 사용 지점 4곳(SOW 초안 생성, 검수 가이드 생성, 완료조건→테스트 변환: `gemini-2.5-flash` / 프리랜서 화면 번역: `gemini-2.5-flash-lite`)의 원가를 대략 계산하면:

- 프로젝트 1건(SOW~마일스톤 8개 검수 완료)당 AI 원가 약 **$0.5**
  - 이 값은 gpt-4o 기준으로 잡은 추정치다. 2026-08-23 Gemini로 바꾸면서 단가가 내려갔으므로 실제 원가는 이보다 낮다. 정확한 값은 `npm run eval:sow`의 토큰 사용량으로 다시 재야 한다.
