...
- ~~프로젝트 개수 기준 티어~~ → **검수(자동 검증) 실행 횟수 기준 3단계 티어제로 변경** ([`src/config/subscription-plan.ts`](../src/config/subscription-plan.ts)). 프로젝트 개수로 과금하면 검수 기능을 한 번도 안 써도 프로젝트만 여러 개 만들면 비싼 플랜을 추천받는 문제가 있었다 — "첫 허들을 낮추자"는 목표에 안 맞아서, 실제로 비용이 드는 자동 검수를 얼마나 썼는지로 기준을 바꿨다.
- **프로젝트 등록, 지원자 모집, SOW 협의는 구독 여부와 무관하게 전부 무료다.** 돈이 드는 건 GitHub 연동 자동 검수(Playwright 실행)뿐이라, 이 기능을 실제로 쓸 때만 과금 대상이 된다.

| 플랜 | 기준 (이번 달 검수 실행 횟수) | 월 금액 |
|---|---|---|
| Starter | 60회 이하 | 49,000원 |
| Growth | 61~180회 | 79,000원 |
| Scale | 181회 이상 | 129,000원 |

- 회사가 소유한 프로젝트들의 **이번 달(캘린더 월 기준) `verification_runs` 행 개수**를 세서 추천 플랜을 계산하고(`recommendedPlanId`), `/company/settings`에서 3개 플랜 카드와 함께 보여준다. 마일스톤 재검수(revision)도 각각 새 실행으로 잡히므로, 마일스톤 하나를 여러 번 재검수하면 횟수가 그만큼 늘어난다.
...
- **이번 스코프에서는 구독 상태가 아무것도 막지 않는다.** 검수 기능 자체도 구독 여부와 무관하게 지금은 그대로 쓸 수 있다 — 나중에 "미구독 상태에서 검수 요청 시 차단" 같은 로직을 붙일 확장 지점으로만 존재한다.
...
즉 가격은 원가가 아니라 "PM/QA 인력 없이 외주를 검증하는 가치"로 매기는 게 맞다. COGS 대비 최소 10배 마진이 SaaS 업계 관행인 걸 감안해서, 검수를 많이 쓸수록(=플랫폼 의존도가 높아질수록) 가격도 같이 올라가는 3단계 구조로 만들었다. 구체적 액수(Starter 49,000원 / Growth 79,000원 / Scale 129,000원)는 정교한 산정이라기보다 "이 정도가 익숙한 가격대"라는 감으로 정한 참고치이고, 실행 횟수 구간(60/180)도 "마일스톤당 재검수까지 감안해서 넉넉하게" 잡은 감각치다. MVP가 아직 시장 검증 전이라, 초기 몇 달은 무료 체험 또는 더 낮은 가격으로 어답션을 우선하는 것도 고려할 만하다.
