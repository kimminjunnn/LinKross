...
export default async function FreelancerInvoicesPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl pb-16">
      ...
      {!result.ok ? (
        <div className="mt-8 flex gap-3 rounded-card border border-danger/30 bg-danger/10 p-5 text-sm font-bold text-danger">
        ...
        <div className="mt-8 rounded-card border border-dashed border-app-border-strong p-10 text-center">
          ...
          <h2 className="mt-3 font-black text-app-foreground">No invoices submitted</h2>
              ...
              <article key={invoice.id} className="p-5 sm:flex sm:items-center sm:justify-between sm:gap-5">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="font-mono text-xs font-bold text-app-muted">{invoice.invoiceNumber}</span>
                    <span className="rounded-full bg-app-surface-subtle px-2.5 py-1 text-xs font-black text-app-muted">{invoice.status}</span>
                  </div>
                  <h2 className="mt-2 font-black text-app-foreground">{invoice.milestoneCode} · {invoice.milestoneTitle}</h2>
                  <p className="mt-1 text-sm text-app-muted">{invoice.organizationName} · {invoice.projectTitle}</p>
                  {invoice.reviewNote && <p className="mt-2 text-xs font-bold text-warning">Client note: {invoice.reviewNote}</p>}
                </div>
                <div className="mt-4 sm:mt-0 sm:text-right">
                  <p className="text-lg font-black text-app-foreground">{invoice.amount.toLocaleString()} {invoice.currency}</p>
