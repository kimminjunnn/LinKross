...
export default async function FreelancerHomePage() {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl pb-16">
      ...
      <h1 className="mt-2 text-3xl font-black text-app-foreground">Welcome back, {display?.name ?? "Freelancer"}</h1>
        ...
        {stats.map((stat) => {
          ...
          return (
            <Link key={stat.label} href={stat.href} className="rounded-card border border-app-border bg-app-surface p-5 shadow-card transition hover:border-brand-300">
              ...
              <p className="mt-4 text-xs font-bold text-app-muted">{stat.label}</p>
              <p className="mt-1 text-3xl font-black text-app-foreground">{stat.value}</p>
              <p className="mt-1 text-xs font-semibold text-app-muted">{stat.detail}</p>
            </Link>
          );
        })}
      </section>
      ...
      <section className="mt-6 rounded-card border border-app-border bg-app-surface p-6 shadow-card">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <Link href="/opportunities" className="flex items-center justify-between rounded-control border border-app-border p-4 text-sm font-bold text-app-foreground hover:bg-app-surface-subtle"><span className="inline-flex items-center gap-2"><Search className="size-4 text-brand-600" />Find a project</span><ArrowRight className="size-4" /></Link>
          <Link href="/freelancer/projects" className="flex items-center justify-between rounded-control border border-app-border p-4 text-sm font-bold text-app-foreground hover:bg-app-surface-subtle"><span className="inline-flex items-center gap-2"><FolderKanban className="size-4 text-brand-600" />Continue selected work</span><ArrowRight className="size-4" /></Link>
        </div>
      </section>
    </div>
  );
