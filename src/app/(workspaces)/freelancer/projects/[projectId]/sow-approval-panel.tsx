    ...
    <div className="mt-5 space-y-4">
      ...
      {errorMessage ? (
        <div className="flex gap-2 rounded-control border border-danger/30 bg-danger/10 p-3 text-danger">
          <CircleAlert className="size-4 shrink-0" /><p className="text-xs font-bold">{errorMessage}</p>
        </div>
      ) : null}
      ...
      <button
        ...
        className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-control bg-brand-500 px-4 text-sm font-bold text-white hover:bg-brand-600 disabled:cursor-not-allowed disabled:opacity-50"
      >
      ...
      {!isApproved && state.status === "in_review" && (
        <div className="space-y-2 rounded-control border border-app-border p-3">
          <label htmlFor="sow-revision-reason" className="text-xs font-black text-app-foreground">Request changes instead</label>
          ...
          <button type="button" onClick={requestRevision} disabled={isPending || !revisionReason.trim()} className="min-h-10 w-full rounded-control border border-app-border-strong px-4 text-sm font-black text-app-foreground disabled:opacity-50">Request SOW revision</button>
        </div>
      )}
      {state.status === "revision_requested" && <p className="rounded-control bg-warning/10 p-3 text-xs font-bold text-warning">Revision requested. This reviewed version remains immutable; the client must prepare a new version.</p>}
...
function ApprovalRow({ label, complete }: { label: string; complete: boolean }) {
  return (
    <div className="flex items-center justify-between rounded-control border border-app-border p-3">
      <span className="text-sm font-bold text-app-foreground">{label}</span>
      <span className={`inline-flex items-center gap-1 text-xs font-bold ${complete ? "text-success" : "text-app-muted"}`}>
