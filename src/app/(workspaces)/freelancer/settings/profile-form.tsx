...
export function FreelancerProfileForm({ initialProfile }: { initialProfile: FreelancerProfileSettings }) {
  ...
  return (
    <form action={(formData) => startTransition(async () => {
      ...
      <button disabled={pending} className="primary-action mt-5 inline-flex min-h-11 items-center gap-2 rounded-control px-5 text-sm font-semibold">{pending ? <Loader2 className="size-4 animate-spin" /> : <Save className="size-4" />}Save profile</button>
    </form>
  );
}
