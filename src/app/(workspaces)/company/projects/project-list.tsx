    ...
    <div className="space-y-4">
      {projects.map((project) => {
        ...
        return (
          <article
            key={project.id}
            className="group rounded-card border border-app-border bg-app-surface p-5 transition-colors hover:border-app-border-strong sm:p-6"
          >
            <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div className="min-w-0 space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  <span className={`inline-flex items-center rounded-control border px-2.5 py-0.5 text-xs font-semibold ${
                    isCompleted
              ...
              <div className="flex items-center gap-3 self-start lg:self-center shrink-0">
                {project.lifecycleStage === "preparing" ? (
                  <Link
                    href={nextHref}
                    className="inline-flex h-10 shrink-0 items-center justify-center gap-1.5 rounded-control border border-brand-200 bg-brand-50 px-5 text-xs font-semibold text-brand-700 transition-colors hover:border-brand-300 hover:bg-brand-100 sm:text-sm"
                  >
                    업무 명세서 확인
