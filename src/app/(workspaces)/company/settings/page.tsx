...
export default async function SettingsPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl">
      ...
      {!result.ok ? <div className="mt-7 flex gap-3 rounded-card border border-danger/30 bg-danger/10 p-5 text-sm font-bold text-danger"><CircleAlert className="size-5 shrink-0" />{result.error.message}</div> : <CompanyProfileForm initialProfile={result.data} />}
      <section className="mt-5 rounded-card border border-app-border bg-app-surface p-5 shadow-card">
        <h2 className="font-black text-app-foreground">외부 연동</h2>
