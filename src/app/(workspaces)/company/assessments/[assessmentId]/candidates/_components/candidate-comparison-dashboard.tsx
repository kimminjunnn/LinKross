...
export function CandidateComparisonDashboard({
  ...
  if (!projectDetail.ok) {
    return (
      <div className="mx-auto w-full max-w-5xl rounded-card border border-danger/30 bg-danger/10 p-6 text-danger">
        <div className="flex gap-3"><CircleAlert className="size-5 shrink-0" /><p className="text-sm">{projectDetail.error.message}</p></div>
      </div>
    );
  }
  ...
  if (!proposals.ok) {
    return (
      <div className="mx-auto w-full max-w-5xl rounded-card border border-danger/30 bg-danger/10 p-6 text-danger">
        <div className="flex gap-3"><CircleAlert className="size-5 shrink-0" /><p className="text-sm">{proposals.error.message}</p></div>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto w-full max-w-7xl pb-16">
      <Link href="/company/assessments" className="inline-flex items-center gap-2 text-sm font-semibold text-app-muted hover:text-brand-700">
      ...
      <section className="mt-6 rounded-card border border-app-border bg-app-surface-subtle p-5">
        <h2 className="text-sm font-semibold text-app-foreground">등록 요구사항</h2>
      ...
      {errorMessage ? (
        <div className="mt-5 flex gap-3 rounded-control border border-danger/30 bg-danger/10 p-4 text-danger">
          <CircleAlert className="size-5 shrink-0" /><p className="text-sm">{errorMessage}</p>
        </div>
      ) : null}
      ...
      {candidates.length === 0 ? (
        <div className="mt-6 rounded-card border border-dashed border-app-border-strong p-10 text-center">
          ...
          <p className="mt-4 text-app-foreground">아직 제출된 수행 제안서가 없습니다.</p>
        </div>
      ) : (
            ...
            return (
              <article key={proposal.id} className={`rounded-card border bg-app-surface p-5 shadow-card ${isSelected ? "border-success/50" : "border-app-border"}`}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h2 className="text-xl font-semibold text-app-foreground">{proposal.freelancer.displayName ?? "이름 미등록 지원자"}</h2>
                ...
                {proposal.freelancer.skills ? (
                  <p className="mt-4 text-xs text-brand-700">{proposal.freelancer.skills}</p>
                ) : null}

                <div className="mt-5 rounded-control border border-app-border bg-app-surface-subtle p-4">
                  <h3 className="text-xs font-semibold uppercase tracking-wider text-app-muted">제출 원문</h3>
                ...
                {proposal.freelancer.portfolioUrls.length ? (
                  <div className="mt-4 flex flex-wrap gap-2">
                    {proposal.freelancer.portfolioUrls.map((url) => (
                      <a key={url} href={url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs font-semibold text-brand-700 hover:underline">
                ...
                <button
                  ...
                  className="mt-5 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-control bg-brand-500 px-4 text-sm font-semibold text-white hover:bg-brand-600 disabled:cursor-not-allowed disabled:opacity-50"
                >
