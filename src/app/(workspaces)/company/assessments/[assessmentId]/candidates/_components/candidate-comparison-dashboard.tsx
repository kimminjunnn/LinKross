...
export function CandidateComparisonDashboard({
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 pb-20">
      ...
      {/* 내가 등록한 공고 원문 */}
      <ProjectPostingPanel projectDetail={projectDetail} />
...
function ProjectPostingPanel({
  ...
  if (!projectDetail.ok) {
    return (
      <div className="rounded-[var(--radius-card)] border border-red-200 bg-red-50 p-4 text-xs font-bold text-red-800">
        {projectDetail.error.message}
      </div>
    );
  }
  ...
  const budgetLabel =
    p.budgetMaxAmount == null
      ? `$${p.budgetAmount.toLocaleString()}`
      : `$${p.budgetAmount.toLocaleString()} ~ $${p.budgetMaxAmount.toLocaleString()}`;

  return (
    <section className="rounded-[var(--radius-card)] border border-app-border bg-app-surface shadow-xs">
      <button
        type="button"
      ...
      >
        <div className="min-w-0">
          <p className="text-xs font-bold uppercase tracking-wide text-app-muted">내가 등록한 공고</p>
          <h2 className="mt-1 truncate text-base font-bold text-app-foreground">{p.title}</h2>
        </div>
        <span className="inline-flex shrink-0 items-center gap-1 text-xs font-bold text-brand-600">
          {expanded ? "접기" : "공고 원문 보기"}
          <ChevronDown className={`h-3.5 w-3.5 transition-transform ${expanded ? "rotate-180" : ""}`} />
        </span>
      </button>

      {expanded ? (
        <div className="space-y-4 border-t border-app-border p-5 text-xs">
          <div className="grid gap-4 sm:grid-cols-2">
            <InfoRow label="유형" value={p.projectType ?? "-"} />
            <InfoRow label="기술 환경" value={p.technology ?? "-"} />
          </div>
          <InfoRow label="목표" value={p.goal} multiline />
          <InfoRow label="핵심 요구사항" value={p.requirements} multiline />
          <div className="grid gap-4 sm:grid-cols-2">
            <InfoRow label="기대 결과물" value={p.deliverables ?? "-"} multiline />
            <InfoRow label="제외 범위" value={p.outOfScope ?? "-"} multiline />
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <InfoRow label="예산" value={budgetLabel} />
            <InfoRow label="개발 일정" value={`${p.startDate} ~ ${p.endDate}`} />
          </div>
          <InfoRow
            label="모집 기간"
            value={`${p.recruitmentStartAt.slice(0, 10)} ~ ${p.recruitmentEndAt.slice(0, 10)}`}
          />
          {p.applicantGuidance ? (
            <InfoRow label="지원자 안내" value={p.applicantGuidance} multiline />
          ) : null}
        </div>
      ) : null}
    </section>
...
function InfoRow({
  ...
  return (
    <div>
      <dt className="text-[11px] font-black uppercase tracking-wide text-app-muted">{label}</dt>
      <dd
