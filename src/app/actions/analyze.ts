...
4. Playwright E2E 테스트용 DoD (완료 기준): 우리는 시스템 검증을 Playwright 자동화 코드로 진행합니다. 따라서 기능 구현뿐만 아니라 디자인/UI 리뉴얼 업무라도 반드시 Playwright 코드로 검증 가능한 구체적인 UI 속성, 상태, 액션 형태(Assertion)로 명확하게 작성해야 합니다. 
   단, 모든 문장의 끝을 무조건 "검증"이라는 단어로 똑같이 끝내지 마세요. "확인", "라우팅됨", "노출됨", "일치함", "통과" 등 문맥에 맞게 자연스럽고 다양하게 마무리하세요.
   - [나쁜 예] 폰트 크기 적절히 조정, 버튼 스타일 통일, 사진 배치 변경
   - [좋은 예] 모바일 뷰포트(375px)에서 상품 상세 본문 텍스트의 computed font-size가 14px 이상인지 확인
   - [좋은 예] 구매 버튼 클릭 시 장바구니 화면으로 정상 라우팅
   - [좋은 예] 상품 이미지 요소(img)의 width가 100%로 렌더링되며 깨짐 현상 없음
